...
<template>
    <div class="flex flex-col items-start">
        <div :class="{'section-title': editable}" class="flex outline-transparent items-center">
            <edit-text v-model="title" class="uppercase text-2xl"/>
            <button v-if="editable" @click="$emit('onAdd')" class="ms-8 hover:ms-0 ps-4 hover:ps-12 text-green-500 pe-4 py-0.5 rounded bg-opacity-20 bg-green-500 hover:bg-opacity-0">
                <icon-list-add class="size-5"/>
            </button>
        </div>
        <div v-if="editable && displayWarning" class="flex items-center text-cyan-500 mt-4 bg-cyan-500 bg-opacity-20 px-2 py-1 rounded">
